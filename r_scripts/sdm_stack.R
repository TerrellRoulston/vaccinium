...
pred_files1 <- file.path(
  ...
  sp_codes1,
  paste0(sp_codes1, "_hist_mask.rds")
)

pred_files2 <- file.path(
  "sdm_output", "sdm_output_feb_10_2026", "masked",
  sp_codes2,
  paste0(sp_codes2, "_hist_mask.rds")
)
...
# Align masked rasters ----------------------------------------------------
# This is an unmasked SDM prediction that I will use as a template raster to align the other rasters too
template <- readRDS("./sdm_output/sdm_output_feb_10_2026/ang/ang_pred_hist.rds")
...
spp_rasters1_aligned <- lapply(spp_rasters1, align_to_template, template = template)
spp_stack1 <- rast(spp_rasters1_aligned)
names(spp_stack1) <- names(spp_rasters1_aligned)  # ang, cor, ...

spp_rasters2_aligned <- lapply(spp_rasters2, align_to_template, template = template)
spp_stack2 <- rast(spp_rasters2_aligned)
names(spp_stack2) <- names(spp_rasters2_aligned)
...
png(file = "./figures/stacked_richness/v2_stacked_vaccinium_richness_hist.png", width = 1600, height = 2000, res = 300)
...
png(file = "./figures/stacked_richness/v2_stacked_corymbosum_richness_hist.png", width = 1600, height = 2000, res = 300)
