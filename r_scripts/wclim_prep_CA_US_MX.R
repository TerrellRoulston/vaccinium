...
# Cropping Worldclim to Canada, Mexico, and US less Hawaii!
# Differs from other Worldclim prep by reducing the number of countries to
# just Canada, US and Mexico
# Terrell Roulston
# Feb 06 2026
...
ssp245_2030_CA_US_MX <- ssp245_2030 %>% 
  tidyterra::select(sel) %>%
  terra::crop(., all_countries_map, mask = TRUE) %>% 
  mask(great_lakes, inverse = T) # cut out the great lakes

# put the names back
names(ssp245_2030_CA_US_MX) <- sel
...
ssp245_2050_CA_US_MX <- ssp245_2050 %>%
  tidyterra::select(sel) %>%
  terra::crop(., all_countries_map, mask = TRUE) %>% 
  mask(great_lakes, inverse = T) # cut out the great lakes

# put the names back
names(ssp245_2050_CA_US_MX) <- sel
...
ssp245_2070_CA_US_MX <- ssp245_2070 %>% 
  tidyterra::select(sel) %>%
  terra::crop(., all_countries_map, mask = TRUE) %>% 
  mask(great_lakes, inverse = T) # cut out the great lakes

# put the names back
names(ssp245_2070_CA_US_MX) <- sel
...
ssp585_2030_CA_US_MX <- ssp585_2030 %>% 
  tidyterra::select(sel) %>%
  terra::crop(., all_countries_map, mask = TRUE) %>% 
  mask(great_lakes, inverse = T) # cut out the great lakes

# put the names back
names(ssp585_2030_CA_US_MX) <- sel
...
ssp585_2050_CA_US_MX <- ssp585_2050 %>%
  tidyterra::select(sel) %>%
  terra::crop(., all_countries_map, mask = TRUE) %>% 
  mask(great_lakes, inverse = T) # cut out the great lakes

# put the names back
names(ssp585_2050_CA_US_MX) <- sel
...
ssp585_2070_CA_US_MX <- ssp585_2070 %>% 
  tidyterra::select(sel) %>%
  terra::crop(., all_countries_map, mask = TRUE) %>% 
  mask(great_lakes, inverse = T) # cut out the great lakes

# put the names back
names(ssp585_2070_CA_US_MX) <- sel

# Saved formatted Predictor Rasters ---------------------------------------
out_dir <- "C:/Users/terre/Documents/R/vaccinium/wclim_data/wclim_CA_US_MX" # specify output dir
