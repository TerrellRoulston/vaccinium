...
mx_map <- gadm(country = 'MX', level = 1, resolution = 2,
               path = './maps/base_maps/') #Canada basemap w. Provinces

canUSmx_map <- rbind(us_map, ca_map, mx_map) #combine US and Canada vector map
...
occ_cor_clean <- occ_cor %>% 
  ...
  cc_sea()%>% 
  distinct(decimalLatitude, decimalLongitude, `ï..gbifID`, .keep_all = T) %>% 
  filter(!(`ï..gbifID`  %in% c('4926151230', '4921982592')))   # 4926151230 IDd incorrectly, is cultivated non-wild
